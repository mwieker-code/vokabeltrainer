...
/* Die Buchangabe ist in allen Jahrgaengen leer. Wo sie ohne Pruefung
   eingesetzt wird, steht auf jeder Karte "Year 5 () - alle Units" und
   in der Uebersicht eine leere Zeile neben dem Jahrgang. */
{
  const fs=require('fs'), path=require('path'), assert=require('node:assert/strict');
  const wurzel=path.resolve(__dirname,'..');
  for(const jahr of ['year5','year6','year7','year8','year9','year10']){
