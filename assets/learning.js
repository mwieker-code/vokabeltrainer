...
Wenn kein geeignetes Paar existiert oder die verlangte Anzahl bzw. Verteilung nicht mit dem ausgewählten Vorrat erreichbar ist: Erstelle noch keinen Test. Erkläre den Engpass und frage nach weniger Opposites, einer größeren Auswahl oder dem Weglassen dieses Aufgabentyps. Falls ausschließlich Opposites gewählt wurden, gilt dieselbe Regel. Erfinde keine Wörter oder Gegensätze und ändere weder Gesamtzahl noch Aufgabenverteilung stillschweigend.\n`:'';
      const matchingFinalCheck=types.includes('matching')
        ...
        :' ';
      /* Die Word Bank stand in einem Year-6-Test in derselben Reihenfolge
         wie die Luecken darunter. Damit war der Test durch blosses
         Abzaehlen zu loesen, ohne ein einziges Wort zu kennen. Die
         Regel steht schon oben beim Aufgabentyp; hier kommt sie in die
         Schlusskontrolle, weil eine Reihenfolge beim Schreiben leicht
         entsteht, ohne dass sie jemand beabsichtigt. */
      const wordBankFinalCheck=wordBank
        ?'Die Word Bank steht alphabetisch nach dem deutschen Begriff und damit nachweislich nicht in der Reihenfolge der Luecken. '
        :'';
...
Prüfe vor Ausgabe Anzahl, einmalige Bewertung jeder Testvokabel, Jahrgangsniveau und Übereinstimmung beider Dateien. Kontrolliere ausdrücklich: name, class und date stehen samt Schreiblinien gemeinsam in einer einzigen Zeile; alle Aufgabenanweisungen sind fett und kursiv.${matchingFinalCheck}${wordBankFinalCheck}${oppositesFinalCheck}Liefere bei nicht erfüllbaren Vorgaben zuerst eine Rückfrage statt eines abweichenden Tests. Falls du keine DOCX-Dateien erzeugen kannst, sage dies ausdrücklich und liefere Test und Lösungen getrennt als kopierbaren Text.
