  ...
  function testPromptDialog(){
    ...
    dialog.innerHTML='<form id="promptConfig"><h2>Welche Aufgabentypen soll dein Vokabeltest enthalten?</h2><p>'+ (upper?'Oberstufe':'Year '+year)+' · '+rows.length+' ausgewählte Vokabeln stehen zur Verfügung.</p>'+(upper?'<label for="promptStage">Zielstufe</label><select id="promptStage"><option value="E">Einführungsphase (E)</option><option value="Q1">Q1</option><option value="Q2">Q2</option></select><p>Bei gemischter Auswahl bitte die gewünschte Zielstufe festlegen.</p>':'')+'<fieldset><legend>Aufgabentypen</legend>'+[['gap','Gap-Filling'],['matching','Definition-Matching'],['opposites','Opposites – beide Wörter müssen in der Auswahl stehen'],['sentences','Eigene Sätze bilden']].map(([v,t])=>'<label><input type="checkbox" name="taskType" value="'+v+'" '+(v==='gap'||v==='matching'?'checked':'')+'> '+t+'</label>').join('')+'</fieldset><label for="promptCount">Anzahl der Vokabeln im Test</label><input id="promptCount" type="number" min="1" max="'+rows.length+'" step="1" value="'+Math.min(20,rows.length)+'" required><p>Du kannst 1 bis '+rows.length+' Vokabeln verwenden. Die KI wählt die gewünschte Anzahl aus deiner Auswahl.</p><p id="promptError" role="alert"></p><button class="primary" type="submit">Prompt erstellen</button> <button type="button" id="promptClose">Schließen</button></form><section id="promptResult" hidden><label for="promptText">Dein fertiger Prompt</label><textarea id="promptText" rows="14" readonly></textarea><button id="promptCopy" type="button">Prompt kopieren</button><p id="promptCopyStatus" role="status"></p><a href="https://chatgpt.com/" target="_blank" rel="noopener noreferrer">ChatGPT öffnen</a> · <a href="https://claude.ai/" target="_blank" rel="noopener noreferrer">Claude öffnen</a><p>Im Chat einfügen und absenden. Die Erstellung herunterladbarer Dateien hängt von den Funktionen und Nutzungslimits des Chatdienstes ab.</p></section>';
    {
      ...
      const sync=()=>{
        ...
        dialog.querySelector('#allocationTotal').textContent=sum+' von '+dialog.querySelector('#promptCount').value+' Wörtern verteilt';
      };
...
- Alle Aufgabenanweisungen unter den Aufgabenüberschriften sind zwingend gleichzeitig fett UND kursiv formatiert (echte Word-Zeichenformatierung, keine sichtbaren Markdown-Sternchen). Insbesondere lautet die Anweisung für Definition-Matching exakt: "Read each definition and write the matching word from the vocabulary list on the line." Keine Umformulierung. Definitionen, Beispielsätze und Antwortzeilen bleiben normal gesetzt.
...
Prüfe vor Ausgabe Anzahl, einmalige Bewertung jeder Testvokabel, Jahrgangsniveau und Übereinstimmung beider Dateien. Kontrolliere ausdrücklich: name, class und date stehen samt Schreiblinien gemeinsam in einer einzigen Zeile; alle Aufgabenanweisungen sind fett und kursiv; die Definition-Matching-Anweisung stimmt Zeichen für Zeichen mit dem vorgeschriebenen englischen Wortlaut überein. Definition-Matching enthält nur Definitionen und leere Antwortlinien, keine vorgegebenen Lösungswörter. Bei jedem Opposites-Paar sind Hinweiswort UND Lösungswort in der ausgewählten JSON-Liste nachweisbar, bedeutungsmäßig gegensätzlich und für andere Aufgaben gesperrt. Dokumentiere die verwendeten Gegensatzpaare mit beiden Listeneinträgen ausschließlich im Answer Key. Liefere bei nicht erfüllbaren Vorgaben zuerst eine Rückfrage statt eines abweichenden Tests. Falls du keine DOCX-Dateien erzeugen kannst, sage dies ausdrücklich und liefere Test und Lösungen getrennt als kopierbaren Text.
