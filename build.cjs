...
for(const item of ['index.html','year8','year5','year7','year6','year9','year10','oberstufe','all','assets']){
 fs.cpSync(path.join(root,item),path.join(out,item),{recursive:true});
}
for(const dir of ['', 'year8','year5','year7','year6','year9','year10','oberstufe','oberstufe/analysis','all','all/analysis','year6/grammar','year6/grammar/simple-past','year6/grammar/irregular-verbs']){
...
console.log('Static build: fourteen pages, script syntax and local links checked.');

for(const f of ['data.js','app.js'])new vm.Script(fs.readFileSync(path.join(out,'all/analysis',f),'utf8'));

for(const f of ['data.js','app.js'])new vm.Script(fs.readFileSync(path.join(out,'year6/grammar/simple-past',f),'utf8'));
