...
for(const item of ['index.html','year5','year6','year9','year10','oberstufe','all','assets']){
 fs.cpSync(path.join(root,item),path.join(out,item),{recursive:true});
}
for(const dir of ['', 'year5','year6','year9','year10','oberstufe','oberstufe/analysis','all','all/analysis','year6/grammar','year6/grammar/simple-past','year6/grammar/irregular-verbs']){
...
console.log('Static build: twelve pages, script syntax and local links checked.');
